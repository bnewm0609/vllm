...
class ModelRunner:
    ...
    def execute_model(
        ...
        if self.return_hidden_states_up_to_layer >= 0:
            # this is just getting the FINAL hidden states
            # to get everything, we'd need to do something like "pad_packed_sequence"
            # which we don't need
            for i, selected_token_i in enumerate(
                sampling_metadata.selected_token_indices.cpu()
            ):
                output[i].hidden_states = [layer[selected_token_i] for layer in hs_list]
